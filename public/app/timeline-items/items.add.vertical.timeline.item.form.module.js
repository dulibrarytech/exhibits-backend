...
const itemsAddVerticalTimelineItemFormModule = (function () {
    ...
    obj.create_timeline_item_record = async function() {
        ...
        if (item_id) {
            ...
            console.error('ERROR: update_timeline_item_record function not found!');
            return false;
        }
        ...
        try {
            ...
            if (!exhibit_id || !timeline_id) {
                display_status_message(message_element, 'warning', 'Missing exhibit ID or timeline ID. Cannot create timeline item record.');
                return false;
            }

            // Show loading state
            display_status_message(message_element, 'info', 'Creating timeline item record...');
            ...
            const endpoint = EXHIBITS_ENDPOINTS.exhibits.timeline_item_records.post.endpoint
            ...
            if (!response || response.status !== 201) {
                if (!response) {
                    display_status_message(message_element, 'danger', 'Permission denied. You do not have access to add items to this timeline.');
                    return false;
                }
