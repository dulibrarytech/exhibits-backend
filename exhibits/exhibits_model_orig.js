...
const E_RECORD_TASK = new EXHIBIT_RECORD_TASKS(DB, TABLES);
const I_RECORD_TASK = new EXHIBIT_ITEM_RECORD_TASKS(DB, TABLES);
const H_RECORD_TASK = new EXHIBIT_HEADING_RECORD_TASKS(DB, TABLES);
const G_RECORD_TASK = new EXHIBIT_GRID_RECORD_TASKS(DB, TABLES);
const T_RECORD_TASK = new EXHIBIT_TIMELINE_RECORD_TASKS(DB, TABLES);
/**
...
exports.create_exhibit_record = async function (data) {
    ...
    try {
        ...
        let is_valid = VALIDATE_CREATE_TASK.validate(data);
        ...
        let result = await E_RECORD_TASK.create_exhibit_record(data);
...
exports.get_exhibit_records = async function () {
    ...
    try {
        ...
        return {
            ...
            data: await E_RECORD_TASK.get_exhibit_records()
        };
...
exports.get_exhibit_title = async function (uuid) {
    ...
    try {
        ...
        return {
            status: 200,
            message: 'Exhibit records',
            data: await E_RECORD_TASK.get_exhibit_title(uuid)
        };
...
exports.get_exhibit_record = async function (uuid) {
    ...
    try {
        ...
        return {
            status: 200,
            message: 'Exhibit records',
            data: await E_RECORD_TASK.get_exhibit_record(uuid)
        };
...
exports.get_exhibit_edit_record = async function (uid, uuid) {
    ...
    try {
        ...
        return {
            status: 200,
            message: 'Exhibit records',
            data: await E_RECORD_TASK.get_exhibit_edit_record(uid, uuid)
        };
...
exports.update_exhibit_record = async function (uuid, data) {
    ...
    try {
        ...
        let is_valid = VALIDATE_UPDATE_TASK.validate(data);
        let is_published = 0;
        ...
        let result = await E_RECORD_TASK.update_exhibit_record(uuid, data);
...
exports.delete_exhibit_record = async function (uuid) {
    ...
    try {
        ...
        let results = await I_RECORD_TASK.get_item_records(uuid);
        ...
        if (results.length > 0) {
            ...
            for (let i=0; i<result.data.length; i++) {
                ...
                let is_deleted = await I_RECORD_TASK.delete_item_record(result.data[i].is_member_of_exhibit, result.data[i].uuid, result.data[i].type);
        ...
        if (await E_RECORD_TASK.delete_exhibit_record(uuid) === true) {
            return {
                status: 204,
...
exports.delete_media_value = async function (uuid, media) {
    ...
    try {
        ...
        if (await E_RECORD_TASK.delete_media_value(uuid, media) === true) {
...
exports.build_exhibit_preview = async function (uuid) {
    ...
    try {
        ...
        let result = await E_RECORD_TASK.set_preview(uuid);
...
const publish_exhibit = async function (uuid) {
    ...
    try {
        ...
        const heading_count = await H_RECORD_TASK.get_record_count(uuid);
        const item_count = await I_RECORD_TASK.get_record_count(uuid);
        const grid_count = await G_RECORD_TASK.get_record_count(uuid);
        const timeline_count = await T_RECORD_TASK.get_record_count(uuid);
        ...
        const is_exhibit_published = await E_RECORD_TASK.set_to_publish(uuid);
        const is_item_published = await I_RECORD_TASK.set_to_publish(uuid);
        const is_heading_published = await H_RECORD_TASK.set_to_publish(uuid);
        const is_grid_published = await G_RECORD_TASK.set_to_publish(uuid);
        const is_timeline_published = await T_RECORD_TASK.set_to_publish(uuid);
        const is_grid_item_published = await G_RECORD_TASK.set_to_publish_grid_items(uuid);
        const is_timeline_item_published =  await T_RECORD_TASK.set_to_publish_timeline_items(uuid);
...
const suppress_exhibit = async function (uuid) {
    ...
    try {
        ...
        const is_exhibit_suppressed = await E_RECORD_TASK.set_to_suppress(uuid);
        const is_item_suppressed = await I_RECORD_TASK.set_to_suppress(uuid);
        const is_heading_suppressed = await H_RECORD_TASK.set_to_suppress(uuid);
        const is_grid_suppressed = await G_RECORD_TASK.set_to_suppress(uuid);
        const is_timeline_suppressed = await T_RECORD_TASK.set_to_suppress(uuid);
        let errors = [];
        ...
        const headings = await H_RECORD_TASK.get_heading_records(uuid);
        const items = await I_RECORD_TASK.get_item_records(uuid);
        const grids = await G_RECORD_TASK.get_grid_records(uuid);
        const timelines = await T_RECORD_TASK.get_timeline_records(uuid);
        ...
        if (is_exhibit_deleted.status === 204) {
            ...
            if (grids.length > 0) {
                ...
                for (let g=0;g<grids.length;g++) {
                    ...
                    await G_RECORD_TASK.set_to_suppressed_grid_items(grids[g].uuid);
            ...
            if (timelines.length > 0) {
                ...
                for (let t=0;t<timelines.length;t++) {
                    ...
                    await T_RECORD_TASK.set_to_suppressed_timeline_items(timelines[t].uuid);
...
exports.delete_exhibit_preview = async function (uuid) {
    ...
    try {
        ...
        const is_exhibit_preview_unset = await E_RECORD_TASK.unset_preview(uuid);
        let errors = [];
...
exports.reorder_exhibits = async function (data) {
    ...
    try {
        ...
        for (let i=0;i<data.length;i++) {
            ...
            is_updated = await E_RECORD_TASK.reorder_exhibits(data[i].type, data[i].order);
