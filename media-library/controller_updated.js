...
/**
 * Checks media library authorization for the current user
 * Supports two-tier permissions: "any" (role-based) vs "own" (ownership-based)
 *
 * @param {Object} req - Express request object (must have req.decoded.sub from TOKEN.verify)
 * @param {string} own_permission - Permission string for own-record access (e.g., 'can_update_media')
 * @param {string} any_permission - Permission string for any-record access (e.g., 'can_update_any_media')
 * @param {string|null} record_created_by - The created_by username from the media record (null for create operations)
 * @returns {Promise<{authorized: boolean, reason: string}>}
 */
const check_media_authorization = async function (req, own_permission, any_permission, record_created_by) {
    ...
    // Check "any" permission first (higher privilege, no ownership check needed)
    if (any_permission) {

        const any_result = await AUTHORIZE.check_permission({
            req,
        ...
        if (any_result === true) {
            return { authorized: true, reason: 'Has any-record permission' };
        }
    }

    // Check "own" permission + ownership
    const own_result = await AUTHORIZE.check_permission({
        req,
    ...
    if (record_created_by === null) {
        return { authorized: true, reason: 'Has create permission' };
    }

    // Verify ownership via created_by username match
    if (String(record_created_by).trim().toLowerCase() === String(username).trim().toLowerCase()) {
        return { authorized: true, reason: 'Record owner' };
    }

    return { authorized: false, reason: 'Not record owner' };
};
...
exports.create_media_record = async function (req, res) {
    ...
    try {
        ...
        // Check media create permission
        const auth_check = await check_media_authorization(
            req,
            'can_create_media',
            null,  // No "any" tier for create — all roles use the same permission
            null   // No ownership check for create
        );

        if (!auth_check.authorized) {
            LOGGER.module().warn(`WARNING: [/media-library/controller (create_media_record)] Authorization denied: ${auth_check.reason}`);
            res.status(403).json({
                success: false,
...
exports.update_media_record = async function (req, res) {
    ...
    try {
        ...
        // Fetch record to check ownership before authorizing
        const existing_record = await MEDIA_MODEL.get_media_record(media_id);

        if (!existing_record || !existing_record.success || !existing_record.record) {
            res.status(404).json({
                success: false,
        ...
        // Check update permission: can_update_any_media (Admin, Power User) or can_update_media + ownership (all roles)
        const auth_check = await check_media_authorization(
            req,
            'can_update_media',
            'can_update_any_media',
            existing_record.record.created_by
        );

        if (!auth_check.authorized) {
            LOGGER.module().warn(`WARNING: [/media-library/controller (update_media_record)] Authorization denied for ${media_id}: ${auth_check.reason}`);
            res.status(403).json({
                success: false,
                message: 'Unauthorized request',
                data: null
...
exports.delete_media_record = async function (req, res) {
    ...
    try {
        ...
        // Check delete permission: can_delete_any_media (Admin) or can_delete_media + ownership (Admin, Power User, General User)
        const auth_check = await check_media_authorization(
            req,
            'can_delete_media',
            'can_delete_any_media',
            existing_record.record.created_by
        );

        if (!auth_check.authorized) {
            LOGGER.module().warn(`WARNING: [/media-library/controller (delete_media_record)] Authorization denied for ${media_id}: ${auth_check.reason}`);
            res.status(403).json({
                success: false,
                message: 'Unauthorized request',
                data: null
