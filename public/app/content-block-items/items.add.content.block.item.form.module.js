...
const itemsAddContentBlockItemFormModule = (function () {
    ...
    obj.create_content_block_record = async function() {
        ...
        try {
            ...
            if (!EXHIBITS_ENDPOINTS?.exhibits?.content_block_records?.post?.endpoint) {
                display_status_message(message_element, 'danger', 'API endpoint configuration missing');
                return false;
            }

            // Construct endpoint with URL encoding
            const endpoint = EXHIBITS_ENDPOINTS.exhibits.content_block_records.post.endpoint
            ...
            console.debug('✅ Item record created successfully, ID:', new_item_id);
    ...
    function redirect_to_item_edit_page(exhibit_id, item_id) {
        console.debug('=== REDIRECTING TO ITEM EDIT PAGE ===');
        ...
        const edit_url = `${APP_PATH}/items/content-block/edit?${params.toString()}`;
    ...
    obj.init = async function () {
        ...
        try {
            ...
            domModule.on('#save-item-btn', 'click', itemsAddContentBlockItemFormModule.create_content_block_record);
