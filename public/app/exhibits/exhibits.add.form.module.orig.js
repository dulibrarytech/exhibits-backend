    ...
    obj.create_exhibit_record = async function () {
        ...
        try {
            ...
            if (heroMediaUuid || thumbnailMediaUuid) {
                ...
                try {
                    const exhibitsFieldEndpoint = APP_PATH + '/api/v1/media/library/record/:media_id/exhibits';

                    if (heroMediaUuid) {
                        const heroUrl = exhibitsFieldEndpoint.replace(':media_id', encodeURIComponent(heroMediaUuid));
                        httpModule.req({
                            method: 'PUT',
                            url: heroUrl,
                            data: { exhibit_uuid: exhibitUuid, action: 'add', media_role: 'hero_image' },
                            headers: {
                                'Content-Type': 'application/json',
                                'x-access-token': token
                            }
                    ...
                    if (thumbnailMediaUuid) {
                        const thumbUrl = exhibitsFieldEndpoint.replace(':media_id', encodeURIComponent(thumbnailMediaUuid));
                        httpModule.req({
                            method: 'PUT',
                            url: thumbUrl,
                            data: { exhibit_uuid: exhibitUuid, action: 'add', media_role: 'thumbnail' },
                            headers: {
                                'Content-Type': 'application/json',
                                'x-access-token': token
                            }
                ...
                } catch (exhibitsFieldError) {
                    console.error('Error updating media exhibits field:', exhibitsFieldError);
            ...
            timeoutId = setTimeout(() => {
                window.location.href = `${APP_PATH}/exhibits/exhibit/details?exhibit_id=${exhibitId}`;
            }, REDIRECT_DELAY);
